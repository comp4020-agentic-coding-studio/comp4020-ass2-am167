...
- Two assessments are marked. The capstone is due next Monday. This is the
  last week for changing the substantive position before Friday’s memo; Monday still allows final board and screenshot preparation.
...
the model is not silent about households. It is thin about
them. Here is exactly how thin.

the Kerrow corridor, a populated Week 4 hypothetical against Week 11:

| Measure | Week 4 hypothetical | Week 11 hypothetical |
| --- | --- | --- |
...
These are worked hypothetical aggregates, not captured game readouts. Land value and rent are indices (Week 4 = 100); livability is a derived worksheet score. The low-wealth count falls by 244, but counts alone cannot distinguish departures from wealth-category changes. The 34 currently homeless households cannot be matched to those 244 without longitudinal records.
...
- The corridor's livability reading went from 61 to 78 across the same
  Week 4–11 interval in this hypothetical. That aggregate gain requires an account of who benefits.
...
- Tenure relations it does not distinguish: ownership (freehold, mortgaged,
  leasehold, co-operative or community land trust), tenancy (fixed-term,
  periodic, public or social with allocation rules), and everything with no
  documented title at all (customary or communal tenure, sub-letting, room
  and boarding, undocumented occupation).
- Nor the instruments attached to them: lease, notice, eviction, mortgage,
  covenant, easement, resumption, compensation, objection, appeal.
...
>
> Livability reads as rising too, because the derived measure is computed
> over the households that are there afterwards.
>
...
1. Who holds the land whose value this plan raised, and what the plan does
   about it.
2. What can the Week 4–11 aggregate counts establish about departures, and where
