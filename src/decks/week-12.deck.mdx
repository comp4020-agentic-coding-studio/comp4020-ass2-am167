...
| Rebuttal | Fails if the bench's fire exposure is judged acceptable and the corridor's loss of 244 low-wealth households is judged unacceptable, in which case the plan's own equity gap is the argument against it |
...
five minutes. The other ten belong to the room, and the
...
```notes
This closes the semester's quantitative loop back to Week 1. The table is the
summary: ask students to identify the column in their own work where the trade
is clearest.
```
...
```notes
Twelve weeks of tradeoffs and the last one is due before lunch.
