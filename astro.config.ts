...
export default defineConfig({
  ...
  integrations: [
    universityTheme({
      ...
      // `site.css` rides the same hook because it carries the layout fixes the
      // brand's own proportions force --- unlayered, so they win over the
      // theme's `@layer at.components` defaults whatever the load order.
